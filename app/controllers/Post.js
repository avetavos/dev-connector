...
const Post_2 = tslib_1.__importDefault(
  require('../middlewares/validations/Post')
);
const Comment_1 = tslib_1.__importDefault(
  require('../middlewares/validations/Comment')
);
  ...
  constructor() {
    ...
    this.likePost = async (req, res) => {
      try {
        ...
        if (
          post.likes.filter(
            like => like.user.toString() === req.user.toString()
          ).length > 0
        ) {
